...
merge_tilesheet :: proc(handler: ^types.IOHandler, first, second: ^types.TileSheet) {
    alloc := virtual.arena_allocator(&handler.arena)

    new_rows := make([][]types.Sprite, len(first.sprites) + len(second.sprites), alloc)
    count := 0

    // copy first
    for r in 0..<len(first.sprites) {
        src := first.sprites[r]
        row := make([]types.Sprite, len(src), alloc) 
        copy(row, src)
        new_rows[count] = row
        count += 1
    }
